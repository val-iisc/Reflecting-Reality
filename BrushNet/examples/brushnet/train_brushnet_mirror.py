...
import cudf.pandas
cudf.pandas.install() # comment these if not in env
import accelerate
import numpy as np
import pandas as pd
import torch
...
from dataset.dataset import HDF5Dataset, MSDDataset
from metrics.metrics import compute_metrics

from ip_adapter.ip_adapter import NormalProjModel, FreqEncoder
...
@torch.no_grad()
def get_normal_embeds(
    normal: torch.Tensor,
    freq_encoder: FreqEncoder,
    normal_proj_model: NormalProjModel,
    do_classifier_free_guidance=True,
):
    """prepare ip_adapter_image_embeds"""
    assert normal.shape == (1, 3), f"Expected normal vector shape (1, 3), got {normal.shape}"
    pos_encoded_normal = freq_encoder(normal) # (1, normals_embeddings_dim)
    normal_embed = normal_proj_model(pos_encoded_normal)  # (1, cross_attention_dim)
    normal_embed = normal_embed.unsqueeze(0) # (1, 1, cross_attention_dim)
    if do_classifier_free_guidance:
        normal_embed = torch.cat([normal_embed, normal_embed], dim=0)  # (2, 1, cross_attention_dim)
    return [normal_embed]
...
def log_validation(
    ...
    def get_hdf5_data(index):
        nonlocal ip_adapter_image_embeds
        ...
        elif args.normals_conditioning_mode == "ip_adapter":
            normal = HDF5Dataset.apply_transforms_normals(data["normals"], mask=data["mask"]).to(accelerator.device)
            ip_adapter_image_embeds = get_normal_embeds(
                normal,
                freq_encoder,
                normal_proj_model,
            )
    ...
    def get_MSD_data(index):
        ...
        orig_mask = np.array(Image.open(Path(args.train_data_dir) / "masks" / image_path))
        masked_image = MSDDataset.get_masked_image(image, orig_mask)

        validation_full_image = Image.fromarray(image, mode="RGB")
        validation_image = Image.fromarray(masked_image, mode="RGB")
        validation_mask = Image.fromarray(orig_mask).convert("RGB")
        depth_image = None
        normal_image = None
        if args.depth_conditioning_mode is not None:
            depth_image = np.load(Path(args.train_data_dir) / "depth" / image_path.replace("png", "npz"))["depth"]
            depth_image = HDF5Dataset.apply_transforms_depth(depth_image, orig_mask)
...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="Simple example of a MirrorFusion training script.")
    parser.add_argument(
        "--pretrained_model_name_or_path",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="BrushNet conditioning scale. (Default 1.0)",
    )
    parser.add_argument(
        "--num_inference_steps",
        type=int,
    ...
    parser.add_argument(
        "--train_base_unet",
        action="store_true",
        help="Whether to train the base generation unet model.",
    )
    parser.add_argument(
        "--hint_map_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=[None, "concat", "latents", "ip_adapter"],
        help=("Normals conditioning mode. Choose between [None, 'concat', 'latents', 'ip_adapter']."),
    )
    parser.add_argument(
        "--ip_adapter_scale",
        type=float,
        default=0.5,
        help="IP Adapter scale for `normals_conditioning_mode` = ip_adapter.",
    )
    parser.add_argument(
        "--cam_states",
        action="store_true",
    ...
    assert 0.0 < args.brushnet_conditioning_scale <= 1.0, "BrushNet conditioning scale should be between 0 and 1"
...
class MirrorFusionModel(nn.Module):
    def __init__(self, unet, brushnet, normal_proj_model, adapter_modules, freq_encoder, weight_dtype=torch.float32):
        super().__init__()
        self.unet: UNet2DConditionModel = unet
        self.brushnet: BrushNetModel = brushnet
        self.normal_proj_model: nn.Module = normal_proj_model
        self.adapter_modules: nn.ModuleList = adapter_modules
        self.freq_encoder: nn.Module = freq_encoder
        self.weight_dtype = weight_dtype

    def get_trainable_modules(self, verbose=True):
        trainable_modules = []
        for name, module in self.named_children():
            # don't save the entire unet in ip_adapter mode
            if name == "unet" and self.normal_proj_model is not None:
                continue
            if any(p.requires_grad for p in module.parameters()):
                if verbose:
                    logger.info(f"Adding {name} to trainable modules")
                trainable_modules.append(module)
        return trainable_modules

    def forward(self, noisy_latents, timesteps, encoder_hidden_states=None, conditioning_latents=None, normal=None):
        ...
        if normal is not None:
            normal_embeds = self.freq_encoder(normal)
            ip_tokens = self.normal_proj_model(normal_embeds)
...
def main(args):
    ...
    if args.brushnet_model_name_or_path:
        ...
        brushnet = BrushNetModel.from_unet(unet, conditioning_channels=conditioning_channels)
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                while len(weights) > 0:
                    weights.pop()
                    model = models[i]
                    if isinstance(model, MirrorFusionModel):
                        for module in model.get_trainable_modules(verbose=False):
                            if isinstance(module, BrushNetModel):
                                sub_dir = "brushnet"
                            elif isinstance(module, UNet2DConditionModel):
                                sub_dir = "unet"
                            elif isinstance(module, NormalProjModel):
                                normal_proj = module.state_dict()
                                continue
                            elif isinstance(module, nn.ModuleList):
                                ip_adapter = module.state_dict()
                                continue
                            else:
                                raise ValueError(f"Unknown model type: {type(module)}")

                            module.save_pretrained(os.path.join(output_dir, sub_dir))
                    else:
                        raise ValueError(f"Unknown model type: {type(model)}")

                    i -= 1

                if normal_proj and ip_adapter:
                    # Save the components into a binary file
                    sub_dir = "ip_adapter"
                    ip_adapter_dir = os.path.join(output_dir, sub_dir)
                    os.makedirs(ip_adapter_dir, exist_ok=True)
                    logger.info(f"Saving ip-adapter to {ip_adapter_dir}")
                    torch.save({"image_proj": normal_proj, "ip_adapter": ip_adapter}, os.path.join(ip_adapter_dir, "ip-adapter.bin"))

        def load_model_hook(models, input_dir):
            def load_and_configure_model(module, input_dir):
                """helper function to load and configure a model"""
                if isinstance(module, BrushNetModel):
                    sub_dir = "brushnet"
                    load_model = BrushNetModel.from_pretrained(input_dir, subfolder=sub_dir)
                elif isinstance(module, UNet2DConditionModel):
                    sub_dir = "unet"
                    load_model = UNet2DConditionModel.from_pretrained(input_dir, subfolder=sub_dir)
                elif isinstance(module, NormalProjModel) or isinstance(module, nn.ModuleList):
                    sub_dir = "ip_adapter"
                    load_model = torch.load(os.path.join(input_dir, sub_dir, "ip-adapter.bin"))
                    if isinstance(module, NormalProjModel):
                        module.load_state_dict(load_model["image_proj"])
                    elif isinstance(module, nn.ModuleList):
                        module.load_state_dict(load_model["ip_adapter"])
                    del load_model
                    return
                else:
                    raise ValueError(f"Unknown model type: {type(module)}")
            ...
            while len(models) > 0:
                ...
                if isinstance(model, MirrorFusionModel):
                    for module in model.get_trainable_modules(verbose=False):
                        load_and_configure_model(module, input_dir)
                else:
                    # allow backwards compatibility to load and resume with older checkpoints
                    load_and_configure_model(model, input_dir)

        accelerator.register_save_state_pre_hook(save_model_hook)
        accelerator.register_load_state_pre_hook(load_model_hook)

    # set various models to eval and train mode
    vae.requires_grad_(False)
    if args.train_base_unet:
    ...
    normal_proj_model = None
    adapter_modules = None
    freq_encoder = None
    # ip-adapter
    if args.normals_conditioning_mode == "ip_adapter":
    ...
    mirror_fusion_model = MirrorFusionModel(
        unet, brushnet, normal_proj_model, adapter_modules, freq_encoder, weight_dtype
    )
    # Get all trainable nn.Modules in the MirrorFusionModel instance
    trainable_modules = mirror_fusion_model.get_trainable_modules()
    ...
    # NOTE: should we check this for all trainable modules?
    ...
    # Optimizer creation
    params_to_optimize = itertools.chain(*[m.parameters() for m in trainable_modules])
    ...
    optimizer, train_dataloader, lr_scheduler, mirror_fusion_model = accelerator.prepare(
        optimizer, train_dataloader, lr_scheduler, mirror_fusion_model
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(mirror_fusion_model):
                ...
                if args.normals_conditioning_mode == "concat":
                    ...
                    normal = batch["normals"].to(dtype=weight_dtype) # normalised mirror normal
                ...
                model_pred = mirror_fusion_model(
                    noisy_latents, timesteps, encoder_hidden_states, conditioning_latents, normal
                )
                ...
                if accelerator.sync_gradients:
                    trainable_modules = mirror_fusion_model.module.get_trainable_modules(verbose=False)
                    params_to_clip = itertools.chain(*[m.parameters() for m in trainable_modules])
    ...
    if accelerator.is_main_process:
        # ip_adapter = None
        # normal_proj = None
        # mirror_fusion_model = unwrap_model(mirror_fusion_model)
        # for module in mirror_fusion_model.get_trainable_modules(verbose=False):
        #     if isinstance(module, BrushNetModel):
        #         module.save_pretrained(os.path.join(args.output_dir, "brushnet"))
        #     elif isinstance(module, UNet2DConditionModel):
        #         module.save_pretrained(os.path.join(args.output_dir, "unet"))
        #     elif isinstance(module, NormalProjModel):
        #         normal_proj = module
        #         continue
        #     elif isinstance(module, nn.ModuleList):
        ...
        # if ip_adapter is not None and normal_proj is not None:
        #     # Save the components into a binary file
        #     sub_dir = "ip_adapter"
        #     ip_adapter_dir = os.path.join(args.output_dir, sub_dir)
        #     os.makedirs(ip_adapter_dir, exist_ok=True)
        #     torch.save({"image_proj": normal_proj.state_dict(), "ip_adapter": ip_adapter.state_dict()}, os.path.join(ip_adapter_dir, "ip-adapter.bin"))
        ...
        if args.num_validation_images > 0:
            image_logs = log_validation(
                ...
                normal_proj_model=normal_proj_model,
                args=args,
                accelerator=accelerator,
