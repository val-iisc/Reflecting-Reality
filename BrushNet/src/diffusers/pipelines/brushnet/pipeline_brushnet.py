...
class StableDiffusionBrushNetPipeline(
    ...
    Args:
        ...
        image_encoder ([`~transformers.CLIPVisionModelWithProjection`], *optional*):
            A `CLIPVisionModelWithProjection` to encode images. If not provided, the `feature_extractor` is used
            instead.
        requires_safety_checker (`bool`, *optional*, default=`True`):
            Whether to use the `safety_checker` or not for NSFW content.
        depth_conditioning_mode (`str`, *optional*):
            The mode to condition the BrushNet guidance on depth maps. Can be one of `None`, `concat`, `latents`.
        normals_conditioning_mode (`str`, *optional*):
            The mode to condition the BrushNet guidance on normal maps. Can be one of `None`, `concat`, `latents`, `ip_adapter`.
    """
    ...
    def __init__(
        ...
        depth_conditioning_mode = None,
        normals_conditioning_mode = None,
    ):
        super().__init__()
        ...
        self.depth_conditioning_mode = depth_conditioning_mode
        self.normals_conditioning_mode = normals_conditioning_mode
    ...
    def prepare_ip_adapter_image_embeds(
        ...
        # NOTE: Do not use this for normals conditioning mode 'ip_adapter'
    ...
    def check_inputs(
        ...
        if (
            ...
            self.check_image(image, mask, prompt, prompt_embeds, depth, normals)
        else:
            assert False
        ...
        if self.normals_conditioning_mode == "ip_adapter" and ip_adapter_image_embeds is None:
            raise ValueError(
                "When `normals_conditioning_mode` is 'ip_adapter', `ip_adapter_image_embeds` must be passed."
            )
    ...
    @staticmethod
    def _is_valid_image_type(obj):
        if isinstance(obj, (PIL.Image.Image, torch.Tensor, np.ndarray)):
            return True
        if isinstance(obj, list):
            return all(isinstance(item, (PIL.Image.Image, torch.Tensor, np.ndarray)) for item in obj)
        return False

    def check_image(self, image, mask, prompt, prompt_embeds, depth, normals):
        if not self._is_valid_image_type(image):
            raise TypeError(
                f"image must be passed and be one of PIL image, numpy array, torch tensor, list of PIL images, list of numpy arrays or list of torch tensors, but is {type(image)}"
            )

        if not self._is_valid_image_type(mask):
            raise TypeError(
        ...
        if self.depth_conditioning_mode is not None and not self._is_valid_image_type(depth):
            raise TypeError(
                f"depth must be one of PIL image, numpy array, torch tensor, list of PIL images, list of numpy arrays or list of torch tensors, but is {type(depth)}"
            )

        if self.normals_conditioning_mode in ["concat", "latents"] and not self._is_valid_image_type(normals):
            raise TypeError(
                f"normals must be one of PIL image, numpy array, torch tensor, list of PIL images, list of numpy arrays or list of torch tensors, but is {type(normals)}"
            )
    ...
    def prepare_image(
        ...
        """
        Preprocesses the input image for inference. Takes it to VAE latent dim and repeats if CFG.
        Mostly: [2, 3, H_f, W_f] for 3 channels RGB images, where H_f and W_f are the final height and width
        If 4 dim tensor is passed, only downscaling happens with normalization if un-normalized.
        """
    ...
    def __call__(
        ...
        depth: PipelineImageInput = None, # add depth input
        normals: PipelineImageInput = None, # add normals input
        ...
        **kwargs,
        ...
        Args:
            ...
            mask (`torch.FloatTensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.FloatTensor]`, `List[PIL.Image.Image]`, `List[np.ndarray]`,:
                ...
                each will be paired with each prompt in the `prompt` list. This also applies to multiple BrushNets,
                where a list of image lists can be passed to batch for each prompt and each BrushNet.
            depth (`torch.FloatTensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.FloatTensor]`, `List[PIL.Image.Image]`, `List[np.ndarray]`,:
                    `List[List[torch.FloatTensor]]`, `List[List[np.ndarray]]` or `List[List[PIL.Image.Image]]`):
                The BrushNet input condition to provide depth guidance to the `unet` for generation. If the type is
                specified as `torch.FloatTensor`, it is passed to BrushNet as is. `PIL.Image.Image` can also be
                accepted as an image. The dimensions of the output image defaults to `image`'s dimensions. If height
                and/or width are passed, `image` is resized accordingly. If multiple BrushNets are specified in
                `init`, images must be passed as a list such that each element of the list can be correctly batched for
                ...
                each will be paired with each prompt in the `prompt` list. This also applies to multiple BrushNets,
                where a list of image lists can be passed to batch for each prompt and each BrushNet.
            normals (`torch.FloatTensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.FloatTensor]`, `List[PIL.Image.Image]`, `List[np.ndarray]`,:
                    `List[List[torch.FloatTensor]]`, `List[List[np.ndarray]]` or `List[List[PIL.Image.Image]]`):
                The BrushNet input condition to provide normals guidance to the `unet` for generation. If the type is
                specified as `torch.FloatTensor`, it is passed to BrushNet as is. `PIL.Image.Image` can also be
                accepted as an image. The dimensions of the output image defaults to `image`'s dimensions. If height
                and/or width are passed, `image` is resized accordingly. If multiple BrushNets are specified in
                `init`, images must be passed as a list such that each element of the list can be correctly batched for
        ...
        # 4. Prepare image, mask, depths, normals
        if isinstance(brushnet, BrushNetModel):
            ...
            # since mask being passed is 3 channel and normalized [-1,1] after prepare_image, we need to convert it to 1 channel
            original_mask=(original_mask.sum(1)[:,None,:,:] < 0).to(image.dtype)
            height, width = image.shape[-2:]

            if depth is not None:
                depth = self.prepare_image(
                    image=depth,
                    width=width,
                    height=height,
                    batch_size=batch_size * num_images_per_prompt,
        ...
        if self.depth_conditioning_mode == "concat":
            depth = torch.nn.functional.interpolate(
                depth, size=(conditioning_latents.shape[-2], conditioning_latents.shape[-1])
            )
            conditioning_latents = torch.cat([conditioning_latents, depth], 1)
        elif self.depth_conditioning_mode == "latents":
            depth = depth.repeat(1, 3, 1, 1)
            depth_latents = self.vae.encode(depth).latent_dist.sample() * self.vae.config.scaling_factor
            conditioning_latents = torch.cat([conditioning_latents, depth_latents], 1)

        if self.normals_conditioning_mode == "concat":
            normals = torch.nn.functional.interpolate(
                normals, size=(conditioning_latents.shape[-2], conditioning_latents.shape[-1])
            )
            conditioning_latents = torch.cat([conditioning_latents, normals], 1)
        elif self.normals_conditioning_mode == "latents":
            normals_latents = self.vae.encode(normals).latent_dist.sample() * self.vae.config.scaling_factor
            conditioning_latents = torch.cat([conditioning_latents, normals_latents], 1)
